...
OPTBL 	!WORD	ZERO,ADD,SUB,MUL,DIV,MOD,INCR,DECR		; 00 02 04 06 08 0A 0C 0E
	...
	!WORD	LNOT,LOR,LAND,BIND,LLA,CB,CW,CS			; 20 22 24 26 28 2A 2C 2E
...
OPXTBL  !WORD	ZERO,ADD,SUB,MUL,DIV,MOD,INCR,DECR     		; 00 02 04 06 08 0A 0C 0E
	!WORD	NEG,COMP,BAND,IOR,XOR,SHL,SHR,IDXW     		; 10 12 14 16 18 1A 1C 1E
	!WORD	LNOT,LOR,LAND,BINDX,LLA,CB,CW,CSX		; 20 22 24 26 28 2A 2C 2E
	!WORD	DROP,DUP,PUSHEP,PULLEP,BRGT,BRLT,BREQ,BRNE     	; 30 32 34 36 38 3A 3C 3E
	!WORD	ISEQ,ISNE,ISGT,ISLT,ISGE,ISLE,BRFLS,BRTRU	; 40 42 44 46 48 4A 4C 4E
...
CLOSURX	PLA
        ...
        PLA
        STA     TMPH
        LDA	PPH		; SAVE CALLER'S STRINGS
        PHA
        LDA	PPL
        PHA
        LDA	IFPH		; SAVE CALLER'S FRAME
        PHA
        LDA	IFPL
        PHA
	LDY	#$04
	LDA	(TMP),Y		; USE CLOSURE FRAME
	STA	IFPH
	DEY
	LDA	(TMP),Y
	STA	IFPL
	DEY
	LDA     (TMP),Y		; WITH LAMBDA'S BYTECODE
        STA	IPH
	DEY
	LDA     (TMP),Y
	STA	IPL
	LDY	#2		; EXTRACT PARAM COUNT FROM ENTER OPCODE
	STA	ALTRDON
	LDA	(IP),Y
	BEQ	+		; SKIP COPY IF NO PARAMS
	STA	TMP
	DEY
	LDA	(IP),Y		; CLOSURE FRAME SIZE, INCLUDING PARAMS
	TAY
-	LDA	ESTKH,X
	DEY
	STA	(IFP),Y
	...
	DEC	TMP
	BNE	-
+	PLA			; TRICKY BUT WE MUST NOT LOSE TRACK OF CALLER'S FRAME
	TAY
	PLA
        JSR	+
        PLA
        STA	PPL		; RESTORE CALLER'S STRINGS ETC
        PLA
        STA	PPH
        RTS
+	PHA			; SAVE ON CALLER'S FRAME ON STACK FOR LEAVE/RET
	TYA
	PHA
        LDY	#3		; 3 = SKIP INITIAL ENTER OPCODE
	LDA	#>OPXTBL
	STA	OPPAGE
	SEI
	JMP	FETCHOP
...
BINDX	+INC_IP
	...
	+INC_IP
 	...
 	STA	ALTRDOFF
 	LDY	#4
 	LDA	(TMP),Y		; ADDR HI OF LAMBDA BYTECODE
 	PHA
 	DEY
 	LDA	(TMP),Y		; ...AND LO
 	PHA
	...
	LDA	PPL
	SEC
	SBC	#8		; 1:STR LEN, 3:JSR, 2:FUNC, 2:FRAME... TOTAL: 8
	STA	PPL
	BCS	+
	DEC	PPH
+	LDY	#0
	LDA	#7		; FAKE STRING LEN
	STA	(PPL),Y
	INY
	LDA	#$20		; JSR CLOSURX
	STA	(PPL),Y
	...
	STA	(PPL),Y
	INY
	PLA			; ADDR OF LAMBDA BYTECODE
	STA	(PPL),Y
	INY
	PLA
	STA	(PPL),Y
	INY
	LDA	IFPL		; ADDR OF THIS FRAME
	STA	(PPL),Y
	INY
	LDA	IFPH
	STA	(PPL),Y
	DEX
	LDA	PPL		; LAST, PUT BOUND PTR ON ESTK
	CLC
	ADC	#1		; SKIP STRLEN BYTE
	STA	ESTKL,X
	LDA	PPH
	ADC	#0
	STA	ESTKH,X
